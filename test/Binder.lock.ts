...
import { Binder, BinderNft, MockERC721, RequirementChecker } from "../typechain-types";
...
describe("Lock", function () {
  ...
  it("unstake should lock the nft", async function () {
    ...
    expect(binderDataView.lockedNfts[0].nft.tokenContract).to.equal(polymonNfts.address);
  ...
  it("replace should lock the nft", async function () {
    ...
    expect(binderDataView.lockedNfts[1].nft.tokenContract).to.equal(polymonNfts.address);
  ...
  it("should revert if user claims locked nft before lock period is over", async function () {
    ...
    await expect(binder.connect(user1).claimUnlockedNfts(0, [0])).to.be.reverted;
  ...
  it("should revert if user claims locked nft for a binder he doesn't own", async function () {
    ...
    // claim with differnet user
    await expect(binder.connect(user2).claimUnlockedNfts(0, [0])).to.be.reverted;
  });
  ...
  it("should revert if a invalid index is claimed", async function () {
    ...
    // claim with differnet user
    await expect(binder.connect(user2).claimUnlockedNfts(0, [1])).to.be.reverted;
  });
  ...
  it("should be able to claim multiple nfts at once", async function () {
    ...
    for (let i = 0; i < 4; i++) {
      expect(binderDataView.lockedNfts[i].nft.tokenContract).to.equal(polymonNfts.address);
  ...
  it("should revert if the indexes are not sorted in decending order", async function () {
    ...
    await expect(binder.connect(user1).claimUnlockedNfts(0, [3, 0, 1])).to.be.reverted;
  });

  it("revert if the lock time gets changed by non GOVERNANCE_ROLE user", async function () {
    await expect(binder.connect(user1).setUnstakeLockPeriod(100)).to.be.reverted;
  });
